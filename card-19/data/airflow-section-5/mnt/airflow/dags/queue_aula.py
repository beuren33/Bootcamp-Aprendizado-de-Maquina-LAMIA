...
with DAG(dag_id='queue_dag', schedule_interval='0 0 * * *', default_args=default_args, catchup=False) as dag:
    ...
    # tasks de I/O vao pra fila do worker com disco SSD
    ...
    # tasks pesadas de CPU vao pra fila do worker de CPU
    ...
    # task que depende do spark, roda na fila dedicada do spark
    t_6_spark = BashOperator(task_id='t_6_spark', bash_command='echo "Spark dependency task"', queue='worker_spark')

    # task final, so espera todas as anteriores terminarem
